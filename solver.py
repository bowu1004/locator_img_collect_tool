...
class Solver(object):
    ...
    def test(self, ckpt_path, batch_size, test_thread_num, img):
        with torch.no_grad():
            self.net.eval()
            ...
            transformed_image = torch.unsqueeze(img_transform(image_pil), 0)


            predictions = self.net(transformed_image.cuda())

            batch_preds = np.squeeze(np.squeeze(np.array(predictions.permute(0, 2, 3, 1).cpu() * 255), 3), 0)  # [N, 1, 224, 224]
        # return cv2.cvtColor(batch_preds, cv2.COLOR_GRAY2RGB)
