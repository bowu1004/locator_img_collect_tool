...
def run_yolov8_inference(faiss_path,out_path):
    ...
    # out = cv2.VideoWriter(out_video_path, fourcc, 30.0, (D400_imgWidth, D400_imgHeight),True)
    ...
    while True:
        ...
        if len(frame_data):
            ...
            origin_image_path = os.path.join(out_path, "origin_image_data")
            if not os.path.exists(origin_image_path):
                os.makedirs(origin_image_path)
            file_path = os.path.join(origin_image_path, filename)
            cv2.imwrite(file_path, origin_frame_data)
            ...
            for bbox in ori_list:
                ...
                filename_xiaotu = generate_random_filename()
                filepath_xiaotu = os.path.join(out_path, "small_image_data")
                if not os.path.exists(filepath_xiaotu):
                    os.makedirs(filepath_xiaotu)
                file_path_xiaotu = os.path.join(filepath_xiaotu, filename_xiaotu)
                cv2.imwrite(file_path_xiaotu, cr_img_origin)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--faiss_path", type=str, default=r"./model_path/faiss_model", help="faiss vector library path")
    parser.add_argument("--out_path", type=str, default=r"./output/data", help="output path")
    # parser.add_argument("--out_video_path", type=str, default=r"./output/video.mp4", help="output video path")
